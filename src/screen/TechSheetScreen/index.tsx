...
import { ORDER_STATE_LABELS } from "../MyOrdersScreen/logic/orderFlow";
const db = firestore();
      ...
      >
        ...
        <View style={styles.topSection}>
          ...
          <View style={styles.topInfo}>
            ...
            <Text style={styles.text}>
              {s?.garment ? i18n.t(s.garment) : i18n.t("not_specified")}
            </Text>
            ...
            <Text style={styles.text}>
              {order.status
                ? i18n.t(
                    ORDER_STATE_LABELS[
                      order.status as keyof typeof ORDER_STATE_LABELS
                    ],
                  )
        ...
        <View style={styles.twoColumns}>
          ...
          <View style={styles.column}>
            ...
            {order.tailor?.email && (
              <Text style={styles.text}>
                {`${i18n.t("email")}: ${order.tailor.email}`}
              </Text>
            )}
          </View>
        </View>
          ...
          <View style={styles.column}>
            ...
            {s?.details && Object.keys(s.details).length ? (
              Object.entries(s.details).map(([key, value]) => {
                ...
                const label =
                  typeof value === "number"
                    ? i18n.t(getOptionLabelById(s.garment as any, value))
                    : Array.isArray(value)
                      ? value
                          .map((id) =>
                            i18n.t(getOptionLabelById(s.garment as any, id)),
                          )
                          .join(", ")
                      : String(value);
