...
export function generatePocketRightPath(
  ...
  const reducirWidht = 9 * escala < point.x - x0;

  const width = reducirWidht ? 7 : (point.x - x0 - 2 * escala) / escala;
...
export function generatePocketLeftPath(
  ...
  const reducirWidht = 8 * escala < x00 - point.x;
  const width = reducirWidht ? 6 : (x00 - point.x - 2 * escala) / escala;
  const x64 = point.x,
    y64 = point.y;
...
export function generatePocketPatchRightPath(
  ...
  const reducirLength = 18 * escala < y3 - point.y;
  const length = reducirLength ? 16 : (y3 - point.y - 2 * escala) / escala;
...
export function generatePocketPatchLeftPath(
  ...
  const reducirLength = 18 * escala < y03 - point.y;
  const length = reducirLength ? 16 : (y03 - point.y - 2 * escala) / escala;
  const point2 = getPointAtY(
  ...
  const x65 = x64 + escala * width,
    y65 = y64 + escala;
  const x66 = x65 - (width - 1.5) * escala,
...
export function generatePocketWeltRightPath(
  ...
  const reducirWidht = 10 * escala < point.x - x0;

  const width = reducirWidht ? 8 : (point.x - x0 - 2 * escala) / escala;
  ...
  return [
    ...
    `C ${x61}, ${y61}, ${x62}, ${y62}, ${x62} ${y62}`,
    `C ${x62}, ${y62 - escala / 8}, ${x63}, ${y63 - escala / 4}, ${x63} ${y63}`,
    `M ${x63} ${y63}`,
    `C ${x63}, ${y63 + escala / 4}, ${x64}, ${y64 + escala / 8}, ${x64} ${y64}`,
...
export function generatePocketWeltLeftPath(
  ...
  const reducirWidht = 9 * escala < x00 - point.x;

  const width = reducirWidht ? 7 : (x00 - point.x - 2 * escala) / escala;
  ...
  return [
    ...
    `C ${x64}, ${y64}, ${x65}, ${y65}, ${x65} ${y65}`,
    `C ${x65}, ${y65}, ${x66}, ${y66}, ${x66} ${y66}`,
    `C ${x66}, ${y66 - escala / 4}, ${x67}, ${y67 - escala / 4}, ${x67} ${y67}`,
    `M ${x67} ${y67}`,
    `C ${x67}, ${y67 + escala / 4}, ${x68}, ${y68 + escala / 8}, ${x68} ${y68}`,
